...
// Mock Neo4jQuery
jest.mock('../neo4jQuery/Neo4jQuery');

describe('GraphMapper', () => {
  let graphMapper: GraphMapper;
  let mockNeo4jQuery: jest.Mocked<Neo4jQuery>;
  let mockSession: any;
  ...
  beforeEach(() => {
    ...
    mockNeo4jQuery = {
      ...
      executeRawQuery: jest.fn().mockResolvedValue({ records: [] }),
      withTransaction: jest.fn(async (callback) => {
        return await callback(mockSession);
      })
    } as unknown as jest.Mocked<Neo4jQuery>;
    
    // Create GraphMapper instance
    graphMapper = new GraphMapper(mockNeo4jQuery, sampleSchema);
  });
  ...
  describe('constructor', () => {
    ...
    it('should create a default TransformerRegistry if none provided', () => {
      const mapper = new GraphMapper(mockNeo4jQuery, sampleSchema);
    ...
    it('should use provided TransformerRegistry if supplied', () => {
      ...
      const mapper = new GraphMapper(mockNeo4jQuery, sampleSchema, customRegistry);
      expect(mapper).toBeDefined();
  ...
  describe('ingest', () => {
    it('should process data and create nodes and relationships', async () => {
      await graphMapper.ingest(sampleData);
      
      // Should call withTransaction
      expect(mockNeo4jQuery.withTransaction).toHaveBeenCalled();
      
      // Verify that session.run was called (don't check exact number of times)
    ...
    it('should handle empty data gracefully', async () => {
      ...
      // Should still call withTransaction but not execute any node or relationship queries
      expect(mockNeo4jQuery.withTransaction).toHaveBeenCalled();
  ...
  describe('createdAt timestamp', () => {
    it('should add createdAt timestamp to all nodes', async () => {
      ...
      // Check all node creation calls include createdAt
      const calls = mockSession.run.mock.calls;
      
      // Check each call that creates a node
      const nodeCalls = calls.filter((call: any) => 
        (call[0].includes('CREATE') || call[0].includes('MERGE')) && 
        call[0].includes('SET')
      );
      
      for (const call of nodeCalls) {
        const propsKey = Object.keys(call[1]).find(k => k.startsWith('props_'));
        expect(propsKey).toBeDefined();
        if (propsKey) {
          const props = call[1][propsKey];
          expect(props.createdAt).toBeDefined();
          expect(neo4j.isDateTime(props.createdAt)).toBeTruthy();
        }
      }
    });
  });
  ...
  describe('JSONPath evaluation', () => {
    it('should correctly evaluate JSONPath expressions for properties', async () => {
      ...
      const jsonPathMapper = new GraphMapper(mockNeo4jQuery, schemaWithJsonPath);
  ...
  describe('serializeSchema and fromSerialized', () => {
    it('should correctly serialize and deserialize schema', () => {
      ...
      const deserializedMapper = GraphMapper.fromSerialized(mockNeo4jQuery, serialized);
  ...
  describe('data mapping', () => {
    ...
    beforeEach(() => {
      ...
      mapDataToGraphMethod = (graphMapper as any).mapDataToGraph.bind(graphMapper);
    });
    ...
    it('should correctly map data to nodes and relationships', async () => {
      ...
      // Check Person nodes
      const personNodes = result.nodes.filter((node: any) => node.type === 'Person');
      ...
      expect(neo4j.isInt(person1?.age)).toBeTruthy();
      ...
      // Check if age is a Neo4j integer
      expect(neo4j.isInt(person2?.age)).toBeTruthy();
      expect(person2?.active).toBe(false);
      expect(person2?.createdAt).toBeDefined();
      
      // Check Organization nodes
      const orgNodes = result.nodes.filter((node: any) => node.type === 'Organization');
      ...
      const worksAtRels = result.relationships.filter((rel: any) => rel.type === 'WORKS_AT');
    ...
    it('should handle nested data structures', async () => {
      ...
      const nestedMapper = new GraphMapper(mockNeo4jQuery, nestedSchema);
      const mapNestedDataMethod = (nestedMapper as any).mapDataToGraph.bind(nestedMapper);
      ...
      // Check Company node
      const companyNodes = result.nodes.filter((node: any) => node.type === 'Company');
      ...
      const deptNodes = result.nodes.filter((node: any) => node.type === 'Department');
      ...
      const hasDeptRels = result.relationships.filter((rel: any) => rel.type === 'HAS_DEPARTMENT');
    ...
    it('should correctly handle type conversions', async () => {
      ...
      const typesMapper = new GraphMapper(mockNeo4jQuery, typesSchema);
      const mapTypesMethod = (typesMapper as any).mapDataToGraph.bind(typesMapper);
      ...
      expect(neo4j.isInt(node.integerValue)).toBeTruthy();
